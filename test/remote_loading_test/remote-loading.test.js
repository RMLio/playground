...
const expectedMappingResponse = await responseFiles['./correct_responses/yarrrml-response.yaml']();
...
// tests for fetching remote mapping rules
describe('loadRemoteMapping()', function() {

  it('should display fetched rules in mapping editor when status=200', async function() {

    expect.assertions(1);

    // initialise mocked server's response for correct URL
    const correctUrl = '/get/existent_mapping';

    xhrMock.get(correctUrl, {
      status: 200,
      body: expectedMappingResponse
    });

    // let matey fetch remote mapping rules and wait for them to be displayed in mapping editor
    await matey.loadRemoteMapping(correctUrl);

    // retrieve value from mapping editor
    const mappingEditorValue = matey.getMapping();

    // check if parsed value equals parsed expected value
    const parsedReceivedValue = parse(mappingEditorValue);
    const parsedExpectedValue = parse(expectedMappingResponse);
    expect(parsedReceivedValue).toEqual(parsedExpectedValue);
  });
  ...
  it ('should display danger alert message when status=404', async function() {
    ...
    // initialise mocked server's response for incorrect URL
    const incorrectUrl = '/get/nonexistent_mapping';
    xhrMock.get(incorrectUrl, {
      status: 404
    });

    // try to fetch remote mapping rules
    await matey.loadRemoteMapping(incorrectUrl);
