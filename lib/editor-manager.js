...
import { createMonacoEditor, init } from "./editor-adapter.js";
export default class EditorManager {
  ...
  async init(config) {
    ...
    this.inputEditorMapping = await createMonacoEditor(
      document.getElementById("editor-mapping-matey"),
      {
        value: "",
        language: "turtle",
   ...
   */
  getInputMapping() {
    return this.inputEditorMapping.getValue();
  }

  setInputMapping(input) {
    return this.inputEditorMapping.setValue(input);
  }
  ...
  createOutputEditor(dataPart, dataType, index = null, selectValue = null) {
    if (!index) {
      index =
        this.outputEditors.length > 0
          ? this.outputEditors[this.outputEditors.length - 1].index + 1
          : 0;
    }
    const editor = this.createAbstractEditor(
      dataPart,
      dataType,
      index,
      selectValue,
  ...
  createAbstractEditor(
    ...
    return {
      ...this.matey.front.createAbstractEditor(
        dataPart,
        dataType,
        index,
        selectValue,
  ...
  createDataEditor(dataPart, index, selectValue = null) {
    ...
    return this.createAbstractEditor(
      temp,
      "Source",
      index,
      selectValue,
  ...
  createAndOpenDataEditor(dataPath, dataValue) {
    ...
    //calculate new index
    const newIndex =
      this.dataEditors.length > 0
        ? this.dataEditors[this.dataEditors.length - 1].index + 1
        : 0;

    //create data editor and set it is the active one
    const dataEditor = this.createDataEditor(
      { path: dataPath, type, value: dataValue },
      newIndex,
    );
  ...
  loadExample(example, reset = false) {
    ...
    this.inputEditorMapping.setValue("");

    function loadRulesEditor(editor, content, reset) {
      if (content) {
        if (Array.isArray(content)) {
          content = content.join("\n"); // the content can be stored as an array of lines for simplicity in the examples file
        }
      } else {
        throw new Error(`this is not a valid example`);
      }
    ...
    loadRulesEditor(this.inputEditorMapping, example.mapping, reset);
    ...
    if (reset) {
      this.matey.front.doAlert(
        `Reloaded the "${example.label}" example`,
        "success",
      );
    }
  }
  ...
  clearAll() {
    ...
    this.destroyDataEditors();
    this.destroyOutputEditors();
    // add default prefixes from config to the mapping editor
    let prefixDeclarations = "";
    if (this.matey.config.defaultPrefixes) {
      for (const [prefix, iri] of Object.entries(this.matey.config.defaultPrefixes)) {
        prefixDeclarations += `@prefix ${prefix}: <${iri}>.\n`;
      }
    }
    this.inputEditorMapping.setValue(prefixDeclarations);
  }
